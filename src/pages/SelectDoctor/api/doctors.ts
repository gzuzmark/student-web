...
const parseResponseData = (doctors: DoctorAvailabilityAPI[]): DoctorAvailability[] =>
	doctors.map(({ schedules, profile_picture, ...rest }: DoctorAvailabilityAPI) => ({
		...rest,
...
export const getMedicalSpecialities = async (data: RequestProps): Promise<DoctorAvailability[]> => {
	...
	// const response = aliviaAxios.get<DoctorAvailability[]>('/doctors/schedules', stringify({ ...requestParams }));
	const parsedData = parseResponseData(mockResponse);
