export default {
	nav: {
		login: 'Iniciar sesión',
	},
	selectDoctor: {
		'left.titlePrefix': 'AHORA,',
		'left.title.firstLine': 'Encuentra al doctor',
		'left.title.secondLine': 'y horario ideal.',
		'right.title': 'Selecciona la fecha y el doctor.',
		'right.dateLabel': 'FECHA:',
		'right.useCaseLabel': 'CASO DE USO:',
		'right.foundDoctors': 'TENEMOS {{doctors}}',
		'right.availableDoctors': 'DOCTORES DISPONIBLES',
		'right.notFoundDoctors': 'NO HAY DOCTORES DISPONIBLES EN LA FECHA SELECCIONADA. POR FAVOR ELIGE OTRA FECHA',
		'steps.firstStep.label': 'DOCTOR & HORARIO',
		'steps.secondStep.label': 'REGISTRO',
		'steps.thirdStep.label': 'RESÚMEN & PAGO',
		'steps.mobile.firstStep.label': 'SOBRE TI',
		'steps.mobile.secondStep.label': 'DATOS MÉDICOS',
		'steps.mobile.thirdStep.label': 'CONTACTO',
	},
};
